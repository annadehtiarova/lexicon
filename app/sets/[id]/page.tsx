...
import { StudySet, VocabWord } from "@/lib/types";
...
const KNOWN_NOUN_ARTICLES = new Map(
  WORD_BANK.filter((word) => word.pos === "noun").map((word) => {
    ...
    return match ? [match[2].toLowerCase(), match[1].toLowerCase()] : [word.german.toLowerCase(), "die"];
  }),
);
...
export default function StudySetPage({
  ...
  const handleMastered = (wordId: string) => {
    setMasteredIds((prev) => {
      ...
      if (set.isPersisted) setMasteredWordIds(id, Array.from(next));
      return next;
    });
  };
      ...
      current
        ? {
            ...
            words: current.words.map((word) =>
              word.id === wordId ? { ...word, german, english } : word,
            ),
          }
        : current,
    );
