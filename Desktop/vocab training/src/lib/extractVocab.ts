...
const GERMAN_FUNCTION_WORDS = new Set(
  "aber als am an auch auf aus bei bin bis das dass dein dem den der des die du ein eine einem einen einer eines er es für gegen haben hat ich im in ist ja kein mit nach nicht nur oder sie sind und vom von war was wir zu zum zur".split(" "),
);
...
function getTranslator() {
  translatorPromise ??= pipeline("translation", "Xenova/opus-mt-de-en");
...
export async function extractVocabFromImages(files: File[]): Promise<ExtractResult> {
  ...
  try {
    ...
    return { topic: "Extracted vocabulary", words };
  } finally {
